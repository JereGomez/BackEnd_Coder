...
class Contenedor{
    ...
    async save(producto){
        ...
        try{
            const contenido = JSON.parse(await fs.promises.readFile(`./${this.nombreArch}.json` , 'utf8')); //OBTENGO ELEMENTOS YA GUARDADOS
            let existe = true;
            while(existe == true){
            ...
            await fs.promises.writeFile(`./${this.nombreArch}.json` , JSON.stringify(contenido.sort((a, b) => a.id - b.id)));
    ...
    async getById(number){
        try{
            const contenido = JSON.parse(await fs.promises.readFile(`./${this.nombreArch}.json` , 'utf8'));//leemos el archivo y obtenemos el contenido en formato JSON
    ...
    async getAll(){
        try{
            const contenido =  await fs.promises.readFile(`./${this.nombreArch}.json` , 'utf8');
    ...
    async deleteById(number){
        ...
        try{
            const contenido = JSON.parse(await fs.promises.readFile(`./${this.nombreArch}.json` , 'utf8'));//leemos el archivo y obtenemos el contenido en formato JSON
            //console.log(contenido);
            await fs.promises.writeFile(`./${this.nombreArch}.json` , JSON.stringify(contenido.filter((item) => item.id != number))); //lo reescribimos filtrando por el ID que deseamos eliminar
    ...
    async deleteAll(){
        ...
        try{
            await fs.promises.writeFile(`./${this.nombreArch}.json` , '');
